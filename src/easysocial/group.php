...
class EasysocialApiResourceGroup extends ApiResource
{
	...
	public function get()
	{
		...
		$groupObject = $EasySocialApiMappingHelper->mapItem([$group], SOCIAL_TYPE_GROUP, $user->id, SOCIAL_TYPE_GROUP);
		...
		$apiResponse->result = $groupObject;
		...
		if ($isNew)
		{
			...
			if ($files->get('avatar'))
			{
				...
				$postValues['avatar'] = array(
					'source' => $avtarMeta['temp_uri'],
					'path' => $avtarMeta['temp_path'],
					'data' => '', 'type' => 'upload',
			...
			$args = array(&$data, 'conditionalRequired' => '', &$stepSession);
			...
			if ($group->state == SOCIAL_STATE_PUBLISHED)
			{
				...
				$this->addTostream($user->id, $group);
