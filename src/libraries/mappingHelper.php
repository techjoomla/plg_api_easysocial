...
class EasySocialApiMappingHelper
{
	...
	public function fieldsSchema($rows, $userid, $type)
	{
		...
		if (count($rows) > 0)
		{
			...
			foreach ($rows as $row)
			{
				...
				$privacy = $lib->validate($element, $fobj->field_id, $elemmentType, $userid);
	...
	public function streamSchema($rows, $userid)
	{
		...
		foreach ($rows as $ky => $row)
		{
			if (isset($row->uid))
			{
				...
				// Added by pratiksha for play uploaded video
				$domVideo = new DOMDocument;
				$domVideo->loadHTML($item->preview);

				foreach ($domVideo->getElementsByTagName('source') as $node)
				{
					$item->source = $node->getAttribute('src');
					break;
				}
	 ...
	 * @since   1.0
	 */
	public function frndNodes($data, $user)
	{
		$frnd_mod = FD::model('Friends');
		$list = array();
